...
import svgPaths from "../../imports/svg-bc64f0ybln";
...
import imgImage28 from "figma:asset/a9c154f9e4e0bb9ccca0f564a5e4d7c35453c7dd.png";
// Resolve kids GIF from src/assets so Vite bundles it correctly
const kidsGif = new URL('../../assets/kids.gif', import.meta.url).href;

// Toggle to enable/disable decorative animations on the cover page.
// The user requested the header/footer/side overlay to be static — set to false.
const ENABLE_COVER_ANIMATIONS = false;

function Subject2() {
  return (
    <motion.div 
      className="absolute h-[1438.92px] left-[1051.11px] top-[653.95px] w-[419.435px]" 
      data-name="Subject 4"
      initial={ENABLE_COVER_ANIMATIONS ? { opacity: 0, x: 100, y: 0 } : { opacity: 1, x: 0, y: 0 }}
      animate={ENABLE_COVER_ANIMATIONS ? { opacity: 1, x: 0, y: [0, -15, 0] } : { opacity: 1, x: 0, y: 0 }}
      transition={ENABLE_COVER_ANIMATIONS ? {
...
function Subject1() {
  return (
    <motion.div 
      className="absolute h-[1649.64px] left-[386.84px] top-[457.27px] w-[497.703px]" 
      data-name="Subject 2"
      initial={ENABLE_COVER_ANIMATIONS ? { opacity: 0, x: -100, y: 0 } : { opacity: 1, x: 0, y: 0 }}
      animate={ENABLE_COVER_ANIMATIONS ? { opacity: 1, x: 0, y: [0, -18, 0] } : { opacity: 1, x: 0, y: 0 }}
      transition={ENABLE_COVER_ANIMATIONS ? {
...
function Subject() {
  return (
    <motion.div 
      className="absolute h-[1495.11px] left-[3.53px] top-[599.76px] w-[487.668px]" 
      data-name="Subject 1"
      initial={ENABLE_COVER_ANIMATIONS ? { opacity: 0, x: -100, y: 0 } : { opacity: 1, x: 0, y: 0 }}
      animate={ENABLE_COVER_ANIMATIONS ? { opacity: 1, x: 0, y: [0, -20, 0] } : { opacity: 1, x: 0, y: 0 }}
      transition={ENABLE_COVER_ANIMATIONS ? {
...
function Subjects() {
  ...
  return (
    <div ref={containerRef} className="absolute h-[2204px] left-[151px] top-[207px] w-[1474px] z-10" data-name="Subjects">
      <div className="absolute h-[177px] left-[-0.48px] top-[1978.48px] w-[1451px]" data-name="image 2">
        <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgImage2} />
      </div>

      {ENABLE_COVER_ANIMATIONS ? (
        <motion.div
          className="absolute inset-0 flex items-center justify-center"
          data-name="kids-gif"
          initial={{ opacity: 0, scale: 0.98 }}
          animate={{ opacity: inView ? 1 : 0.8, scale: inView ? 1 : 0.99 }}
          transition={{ duration: 0.5 }}
        >
          {inView ? (
            <img alt="Kids animation" className="max-w-full h-auto pointer-events-none" style={{ transform: 'scale(1.30) translate(-10px, 120px)' }} src={kidsGif} />
          ) : (
            // static fallback when GIF is out of view to save CPU
            <img alt="Kids static" className="max-w-full h-auto pointer-events-none" src={imgImage1} />
          )}
        </motion.div>
      ) : (
        // Render static GIF/image without animation wrappers
        <div className="absolute inset-0 flex items-center justify-center" data-name="kids-gif-static">
          {inView ? (
            <img alt="Kids animation" className="max-w-full h-auto pointer-events-none" style={{ transform: 'scale(1.30) translate(-10px, 120px)' }} src={kidsGif} />
          ) : (
            <img alt="Kids static" className="max-w-full h-auto pointer-events-none" src={imgImage1} />
      ...
      <div className="flex h-[calc(1px*((var(--transform-inner-width)*1)+(var(--transform-inner-height)*0)))] items-center justify-center relative shrink-0 w-[calc(1px*((var(--transform-inner-height)*1)+(var(--transform-inner-width)*0)))]" style={{ "--transform-inner-width": "74", "--transform-inner-height": "74" } as React.CSSProperties}>
        <div className="flex-none rotate-[90deg]">
          <div className="relative size-[74px]">
            <motion.div 
              className="absolute inset-[-8.11%_-0.32%_20.95%_-0.32%]"
              animate={{
                x: [0, 8, 0],
              }}
              transition={{
...
function Bottom() {
  return (
    <div className={"absolute box-border content-stretch flex flex-col gap-[10px] items-end left-[999.42px] px-0 py-px top-[2129.28px] w-[602.06px] z-20 " + (ENABLE_COVER_ANIMATIONS ? 'animate-[bounce-subtle_4s_ease-in-out_infinite]' : '')} data-name="Bottom" style={{ animationDelay: ENABLE_COVER_ANIMATIONS ? '1s' : undefined }}>
      <Frame2 />
...
function Side() {
  return (
    <div className={"absolute h-[471px] left-0 top-[1682px] w-[879px] z-20 " + (ENABLE_COVER_ANIMATIONS ? 'animate-[shimmer_3.5s_ease-in-out_infinite]' : '')} data-name="Side" style={{ animationDelay: ENABLE_COVER_ANIMATIONS ? '0.5s' : undefined }}>
      <div className="absolute bottom-0 left-0 right-0 top-[-2.52%]">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 879 483">
          <g id="Side">
            ...
            <g filter="url(#filter1_d_1_53)" id="GODâS REALM">
...
function HealingToTheNations() {
  return (
    <div className="absolute h-[366px] left-[0.15px] top-[39.49px] w-[885px] animate-[float-gentle_4s_ease-in-out_infinite]" data-name="Healing to the Nations">
...
function Logo() {
  return (
    <div className={"absolute h-[228.783px] left-0 top-0 w-[232.796px] " + (ENABLE_COVER_ANIMATIONS ? 'animate-[pulse-glow_3s_ease-in-out_infinite]' : '')} data-name="Logo">
...
function Top() {
  return (
    <div className="absolute h-[495.696px] left-[295.01px] top-[144.49px] w-[1165.99px] z-20" data-name="Top">
      <Theme />
      <Logo />
    </div>
  );
...
export function CoverPage() {
  ...
  useEffect(() => {
    ...
    // Resolve the bundled MP4 from src/assets
    const videoSrc = new URL('../../assets/Cover.mp4', import.meta.url).href;
    videoElement.src = videoSrc;
    videoElement.loop = true;
    videoElement.volume = 1; // Full volume for video
  ...
  return (
    <div className="bg-black relative size-full flex items-center justify-center overflow-hidden" data-name="cover-video-page">
      ...
      <motion.button
        ...
        className="fixed bottom-32 left-8 z-50 pointer-events-auto bg-gradient-to-br from-green-600 to-teal-600 text-white p-4 rounded-full shadow-2xl hover:scale-110 transition-transform border-4 border-white"
